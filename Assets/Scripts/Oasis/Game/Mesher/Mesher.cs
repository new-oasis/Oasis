...
using BlockState = Oasis.ECS.BlockStates.BlockState;
using Texture = Oasis.ECS.Textures.Texture;

namespace Oasis.Game.Mesher
{
    public class Mesher
    {
            ...
            for (var z = 0; z < chunkDims.z; z++)
            {
                ...
                for (var side = 0; side < 6; side++)
                {
                    ...
                    if (IsFaceVisible(blockState, otherBlockState))
                    {
                        ...
                        var texture = em.GetComponentData<Texture>(modelFace.Texture);
